...
- **Catching Android up with the 2026-09-16 web session** (Supabase sync for Schedules/Lists, Task Calendar Items/layers, state preservation, Ctrl+T, Backspace history, customizable hotkeys) — most of that work is shared code and needs nothing, but a few concrete gaps (an Android-only quick-add path bypassing the new deadline→reminder wiring, the hardware back button not knowing about the new section-history stack, the new Layers picker having no touch-sheet variant) are scoped out in full, task-by-task, in **`docs/android/06-web-session-catchup.md`** — read that before starting rather than re-deriving the gap list.
- **Drag-and-drop features (List tab reordering, Chronicle tree drag-to-nest, plus the pre-existing NoteEditor note-tab reordering, whose Tauri desktop build the `src-tauri/tauri.conf.json` `dragDropEnabled: false` fix unblocked) have not been assessed for Android at all, and their own CLAUDE.md documentation (search for "drag-and-drop" in the Notes App section) does not yet say anything about Android either.** This is a real, not-yet-investigated risk, not just an unwritten checkbox: native HTML5 drag-and-drop (`draggable`, `dragstart`/`dragover`/`drop`) is a mouse-input model that mobile WebViews generally do not translate from touch gestures without extra work (a touch-to-DnD polyfill, or a hand-rolled `touchstart`/`touchmove`/`touchend` reimplementation) — it may simply not be triggerable by a finger at all in the Android build today. Needs its own investigation pass (does it work, partially work, or not work on the AVD) and its own CLAUDE.md Android-status update once assessed — do not assume it works, and do not conflate this with the separate, already-scoped work in `docs/android/06-web-session-catchup.md`.
