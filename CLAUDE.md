...
Current taskStore v10 migrate backfills: `routineTasks: []`, `repeatConfig: null`, `fieldSchema: []`, `tagIds: []` on collections (v5); `scheduledAt: null`, `scheduledTime: null`, `calendarEventId: null` on tasks (v6); `collectionId: null` on collections (v7); `archivedAt: null` on both collections and purposes (v8); `calendarReminderId: null` on tasks (v9); `crossAppRefs: []` on tasks (v10).
...
| `016_task_cross_app_refs.sql` | `cross_app_refs jsonb not null default '[]'` on `tasks` — reverse cross-app links (e.g. which note(s) a task was created from); see "Cross-app linking" — **written, not yet run against the live project** |
...
- [x] **Cross-app linking — Notes → Task, bi-directional, via a "Create ▸" selection menu (Ctrl+Q).** The suite-wide vision (CLAUDE.md's "Multi-app suite architecture") has always called for cross-app connections; this is the first one actually built end-to-end rather than just reserved-in-the-schema (`cross_app_links` — see below for why that table was deliberately *not* used). Addresses BACKLOG.md's "Cross-app built-in tag types" item with a more general mechanism than that entry originally proposed — see "Relationship to the deferred backlog item" below. Built in two passes within the same session; this entry describes the final state, not the history.
  ...
  - **Non-AI smart pre-fill** (`src/utils/textToTask.ts`, `inferTaskFromSelection(text, now)`): regex-based, no external dependency, locale-aware via `Intl` where genuine ambiguity requires a convention choice. Infers four things:
    ...
    - **Links**: every `http(s)://` or `www.`-prefixed URL found in the text, trailing sentence punctuation trimmed, normalized via the existing `normalizeLinkUrl()`, de-duplicated — feeds `AddTaskModal`'s `links` field via the same prefill mechanism.
  ...
  - **Explicitly not built (see BACKLOG.md for the full, detailed spec of each)**: Calendar item / List item / Tracker entry as real (non-stub) create-menu targets — the same inheritance rules (Endeavour, and Tags once that's designed) are specified to apply to all of them, not just Task; a UI for *manually* linking a Task to an existing note (browsing/searching notes, not just the automatic backlink from creation); AI-assisted inference (the user's stated stretch goal — the regex/Intl-based inference here is the deliberate non-AI first step); bulk/automatic detection of linkable phrases without an explicit selection.
